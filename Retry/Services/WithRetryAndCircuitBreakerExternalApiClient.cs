...
public sealed class WithRetryAndCircuitBreakerExternalApiClient : IExternalApiClient
{
    ...
    private readonly AsyncPolicyAndHandlerWrapper<OneOf<string, NotFound, Error>> _getUserFullNameByIdPolicyAndHandler;
    ...
    public WithRetryAndCircuitBreakerExternalApiClient(IExternalApiClient api, PolicyAndHandlerWrapperProvider provider, ILogger<WithRetryAndCircuitBreakerExternalApiClient> logger)
    {
        ...
        _getUserFullNameByIdPolicyAndHandler = provider.GetRetryAndCircuitBreakerTransientHttpRequestExceptionOrOneOfResultWithNotFoundAsyncPolicyAndHandler<string>(logger);
    }
    ...
    public Task<OneOf<string, NotFound, Error>> GetUserFullNameById(int id, bool fail, CancellationToken cancellationToken) =>
        _getUserFullNameByIdPolicyAndHandler.ExecuteAsResult((fail, _api, id), PolicyAndHandlerWrapperExtensions.CircuitBreakerOpenError, static (p, token) => p._api.GetUserFullNameById(p.id, p.fail, token), cancellationToken);
}